...
class ThreadProxy(threading.Thread):
    ...
    def init_proxy(self,proto,proxy_address):
        proxy_handler = urllib2.ProxyHandler({'%s' % (proto): r"%s://%s" % (proto,proxy_address)})
...
def get_proxy_ip(url):
    try:
        ...
        soup = BeautifulSoup(page)
        links = soup.findAll(attrs={"cellpadding":"0","cellspacing":"1","style":"width:100%; background-color:#ccc;"})
        proto, rest = urllib.splittype(url)
        host,rest= urllib.splithost(rest)
        host = "http://" + host
